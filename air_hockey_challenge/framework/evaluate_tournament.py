...
from air_hockey_challenge.utils.tournament_agent_wrapper import RemoteVsLocalTournamentAgentWrapper, SimpleTournamentAgentWrapper, \
...
def run_remote_vs_local_tournament(
    name_1,
    log_dir,
    build_agent_2=None,
    agent_2_kwargs={},
    ...
    import docker
    BASE_NAME_IMG = "swr.eu-west-101.myhuaweicloud.eu/tud_huawei_org/solution-"
    
    def agent_builder(mdp, i, docker_name_1, build_agent_2, agent_2_kwargs):
        port_1 = 8000
        host_1 = "localhost"
        ...
        container_id_1 = client.api.create_container(BASE_NAME_IMG + docker_name_1, detach=True,
                                                ...
                                                command=f"python 2025-challenge/run.py -e tournament_server --host 0.0.0.0 --port {port_1}")

        container_1 = client.containers.get(container_id_1)

        container_1.start()

        while container_1.status != "running":
            time.sleep(1)
            container_1.reload()
        time.sleep(5)

        def log_docker_out(log_dir, container_1, docker_name_1):
            while True:
                try:
                    container_1.reload()
                    with open(os.path.join(log_dir, f"out.log"), 'ab') as f:
        ...
        thread = Thread(target=log_docker_out, daemon=True, args=(log_dir, container_1, docker_name_1))
        thread.start()
        ...
        return RemoteVsLocalTournamentAgentWrapper(host_1, port_1, container_id_1, name_1, log_dir, agent_2)

    interpolation_order = [3, 3]
    
    # get interpolation order setting from docker container
    client = docker.from_env()
    container = client.containers.create(BASE_NAME_IMG + name_1)
    bits, stats = container.get_archive("/src/2025-challenge/air_hockey_agent/agent_config.yml")
    ...
    for member in tar.getmembers():
        ...
        if "interpolation_order" in config.keys():
            interpolation_order[0] = config["interpolation_order"]
    container.remove()
    ...
    _run_tournament(
        ...
        render,
        save_away_data,
        interpolation_order=tuple(interpolation_order),
        docker_name_1=name_1,
...
def run_remote_tournament(name_1, name_2, log_dir, steps_per_game=45000, n_episodes=1, n_cores=-1, seed=None,
    ...
    Args:
        name_1: Name of the first docker image. Will be appended to the prefix
            "swr.eu-west-101.myhuaweicloud.eu/tud_huawei_org/solution-"
    ...
    def agent_builder(mdp, i, docker_name_1, docker_name_2):
        ...
        container_id_2 = client.api.create_container(BASE_NAME_IMG + docker_name_2, detach=True,
                                                     ...
                                                     command=f"python 2025-challenge/run.py -e tournament_server --host 0.0.0.0 --port {port_2}")
        ...
        def log_docker_out(log_dir, container_1, container_2, docker_name_1, docker_name_2):
            while True:
                try:
                    container_1.reload()
                    with open(os.path.join(log_dir, f"out_{docker_name_1}.log"), 'wb') as f:
                        for line in container_1.logs(stream=True, follow=False):
                            f.write(line)

                    container_2.reload()
                    with open(os.path.join(log_dir, f"out_{docker_name_2}.log"), 'wb') as f:

...
class AirHockeyChallengeWrapper(Environment):
    ...
    def _get_obs(self, data):
        return self.base_env._get_obs(data)
...
if __name__ == "__main__":
    ...
    env = AirHockeyChallengeWrapper(env="defend")
    env.reset()

    action = np.zeros((2, env.env_info['robot']['n_joints']))

    for i in range(5):
        state, reward, done, info = env.step(action)
        print("Step:", i)
        # pprint.pp(info['constraints_value'])
        epy.pprint(info['constraints_value'])